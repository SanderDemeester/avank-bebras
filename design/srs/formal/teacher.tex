...
\begin{section}{Teachers}
    ...
    \end{subsubsection}
    \begin{subsection}{Moving ongoing competition status to different computer}
    When a student is participating in a competition and the computers fails, the teacher has the possiblilty to move the status for that single instance of that competition to a different computer. The teacher must on his account ask for a secret one time token that he/she has to enter on the computer of the student. This token is uniqe for the running competition and is the same for all computers. This token gives the teacher access to an ID that contains all
    \end{subsection}
