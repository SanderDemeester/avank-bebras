\begin{section}{General}
	...
	\begin{subsection}{Default Management Table Form}
		This is a specific page that is available to the organizer, but it is an important
		...
		entering (a part) of the pupil's name and the id of the class. The table will
		automatically update when the search button is pressed and will only contain
		rows that satisfy the input query.
		\\
		\\
		The table won't show all rows at a time, but it will use a paging system that
